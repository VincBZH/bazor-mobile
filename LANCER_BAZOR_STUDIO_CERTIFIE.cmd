...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$cmd=Get-Command py.exe -ErrorAction SilentlyContinue;if($cmd){$py=$cmd.Source;$launchArgs=@('-3',$script)};" ^
  "if(-not $py){$procs=Get-CimInstance Win32_Process -ErrorAction SilentlyContinue;foreach($p in $procs){if(([string]$p.CommandLine) -match 'bazor_pc_relay_v3\.py' -and $p.ExecutablePath){$candidate=[string]$p.ExecutablePath;if($candidate -match 'pythonw\.exe  "if(-not $py){$cands=@($env:LOCALAPPDATA+'\Programs\Python\Python313\python.exe',$env:LOCALAPPDATA+'\Programs\Python\Python312\python.exe',$env:LOCALAPPDATA+'\Programs\Python\Python311\python.exe','C:\Python313\python.exe','C:\Python312\python.exe','C:\Python311\python.exe');foreach($cand in $cands){if(Test-Path $cand){$py=$cand;$launchArgs=@($script);break}}};" ^
...
if "%RC%"=="0" (
  ...
  echo [OK] Ouverture navigateur demandee : http://127.0.0.1:8191/
) else (
...
:END
exit /b %RC%
){$candidate=$candidate -replace 'pythonw\.exe  "if(-not $py){$cands=@($env:LOCALAPPDATA+'\Programs\Python\Python313\python.exe',$env:LOCALAPPDATA+'\Programs\Python\Python312\python.exe',$env:LOCALAPPDATA+'\Programs\Python\Python311\python.exe','C:\Python313\python.exe','C:\Python312\python.exe','C:\Python311\python.exe');foreach($cand in $cands){if(Test-Path $cand){$py=$cand;$launchArgs=@($script);break}}};" ^
  "if(-not $py){Write-Host '[BLOQUE] Python BAZOR introuvable.' -ForegroundColor Red;exit 11};" ^
  "Write-Host ('Python: '+$py);" ^
  "& $py @launchArgs;exit $LASTEXITCODE"
...
:END
exit /b %RC%
,'python.exe'};if(Test-Path $candidate){$py=$candidate;$launchArgs=@($script);break}}}};" ^
